        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[300px]">
          {images.map((img, index) => (
            <div 
              ...
              className={`relative overflow-hidden rounded-3xl border border-white/10 bg-neutral-900 group transition-all duration-500 hover:border-blue-500/40 hover:shadow-[0_0_30px_rgba(59,130,246,0.1)] cursor-pointer ${img.className}`}
            >
              <img 
                src={img.src} 
                alt={img.alt}
      ...
      {selectedImage && (
        ...
        >
          ...
          >
            ...
            {/* Caption */}
            <div className="bg-black/40 backdrop-blur-xl border border-white/10 px-5 py-2.5 rounded-2xl text-white text-xs font-bold uppercase tracking-widest">
              {selectedImage.alt}
            </div>
          </div>
        </div>
